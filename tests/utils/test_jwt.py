...
def test_verify_access_token_signature(dummy_access_token_factory, dummy_payload, dummy_request_for_verify, dummy_private_key_for_verify):
    dummy_access_token = dummy_access_token_factory(dummy_payload)

    with pytest.raises(HTTPException) as exc:
        jwt_helpers.verify_access_token(dummy_request_for_verify, dummy_access_token, dummy_private_key_for_verify)
